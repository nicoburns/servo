...
fn process_offset_parent_query_inner(
        ...
        if fragment.tag() == Some(tag_to_find) {
            ...
            let parent_node_address = match fragment {
                Fragment::Box(fragment) | Fragment::Float(fragment) => {
                    ...
                    let is_static_body_element = is_body_element && fragment.style.get_box().position == Position::Static;
                    match base.tag {
                        Some(tag) if is_eligible_parent && !tag.is_pseudo() => Some((tag.node, is_static_body_element)),
                        _ => None,
                    }
                },
                Fragment::AbsoluteOrFixedPositioned(_) |
    ...
    let offset_parent_padding_box_corner =
        if let Some(offset_parent_node_address) = node_offset_box.offset_parent_node_address {
            // Although the spec (https://www.w3.org/TR/cssom-view-1/#extensions-to-the-htmlelement-interface) says that offsetTop/offsetLeft
            // are always relative to the padding box of the offsetParent. In practice this is not true in major browsers in the case that the
            // offsetParent is the body element and the body element is position:static. In that case offsetLeft/offsetTop are computed relative
            // to the root nodes border box.
            if node_offset_box.is_static_body_element {
                let containing_block = &fragment_tree.initial_containing_block;
                match &(*fragment_tree.root_fragments[0].borrow()) {
                    Fragment::Box(fragment) | Fragment::Float(fragment) => {
                        // Again, take the *first* associated CSS layout box.
                        let border_box_corner = fragment
                            .border_rect()
                            .to_physical(fragment.style.writing_mode, containing_block)
                            .origin
                            .to_vector() +
                    ...
                    Fragment::Image(_) |
                    Fragment::IFrame(_) |
                    Fragment::Positioning(_) => unreachable!(),
                }
            } else {
